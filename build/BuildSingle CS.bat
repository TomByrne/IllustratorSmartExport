...
echo.
echo ================
echo EXPORT START CS: %AppTitle% v%version%
echo ================
echo.
echo.
...
if exist ".\bin\%AppName% CS v%version%.zxp" (
	echo.
	echo.
    echo WARNING: Output file already exists, delete or change version: \bin\%AppName% CS v%version%.zxp
...
:DELETE_EXISTING
del ".\bin\%AppName% CS v%version%.zxp"
if exist "./bin/%AppName% CS v%version%.zxp" (
...
:BUILD
java -jar build/signingtoolkit/ucf.jar -package -storetype PKCS12 -keystore build/cert.p12 -tsa http://timestamp.digicert.com -storepass %password% "./bin/%AppName% CS v%version%.zxp" -C ./%TempDir% .
